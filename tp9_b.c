...
void cargar_parciales(materia_t *p) {
  for (int i = 0; i < p->cantidad_parciales; i++) {
    ...
    do {
      ...
      scanf("%d", &nota);
      if (nota < 1 || nota > 10) {
        printf("Valor inválido. Debe ser entre 1 y 10.\n");
      }
...
void calcular_promedio(materia_t *p) {
  ...
  for (int i = 0; i < p->cantidad_parciales; i++)
    suma += p->parcial[i];
  p->promedio = (p->cantidad_parciales > 0) ?
                (float)suma / (float)p->cantidad_parciales : 0.0f;
}
...
void liberar_parciales(int *p) { if (p) free(p); }
void liberar_materias(materia_t *p) { if (p) free(p); }